...
namespace freeling {
  ...
  namespace semgraph {
    ...
    /// SG_relation_tmp
    SG_relation_tmp::SG_relation_tmp(const std::wstring &rid) {id = rid;}
    SG_relation_tmp::SG_relation_tmp(const std::wstring &rid, const std::wstring &rel, const std::wstring &id1, const std::wstring &id2, const std::wstring &t1, const std::wstring &t2) {
      id= rid; this->rel = rel; w1=id1; w2=id2; this->t1 = t1; this->t2 = t2;
    }
    SG_relation_tmp::~SG_relation_tmp() {}

    std::wstring SG_relation_tmp::get_id() const {return id;}
    std::wstring SG_relation_tmp::get_w1() const {return w1;}
    std::wstring SG_relation_tmp::get_w2() const {return w2;}
    std::wstring SG_relation_tmp::get_t1() const {return t1;}
    std::wstring SG_relation_tmp::get_t2() const {return t2;}
    std::wstring SG_relation_tmp::get_rel() const {return rel;}

    void SG_relation_tmp::set_w1(const std::wstring &id1) {w1 = id1;}
    void SG_relation_tmp::set_w2(const std::wstring &id2) {w2 = id2;}
    void SG_relation_tmp::set_t1(const std::wstring &t1) {this->t1 = t1;}
    void SG_relation_tmp::set_t2(const std::wstring &t2) {this->t2 = t2;}
    void SG_relation_tmp::set_rel(const std::wstring &rel) {this-> rel = rel;}
    ...
    wstring semantic_graph::add_relTemp(SG_relation_tmp &rt) {
      relations.push_back(rt);
      reltemp_id.insert(make_pair(rt.get_id(),relations.size()-1));
      return rt.get_id();
    }
    ...
    const SG_relation_tmp & semantic_graph::get_relation(const std::wstring &rid) const {
      map<wstring,int>::const_iterator rt = reltemp_id.find(rid);
      if (rt == reltemp_id.end())
        ERROR_CRASH(L"RelTemp "+rid+L" does not exist in semantic graph.");
      return relations[rt->second];
    }
    SG_relation_tmp & semantic_graph::get_relation(const std::wstring &rid) {
      map<wstring,int>::iterator rt = reltemp_id.find(rid);
      if (rt == reltemp_id.end())
        ERROR_CRASH(L"RelTemp "+rid+L" does not exist in semantic graph.");
      return relations[rt->second];
    }
    ...
    const std::vector<SG_relation_tmp> & semantic_graph::get_relations() const {return relations;}
    std::vector<SG_relation_tmp> & semantic_graph::get_relations() {return relations;}

    const std::vector<SG_relation_tmp> & semantic_graph::get_relations(const std::wstring &wid) const {
      std::vector<SG_relation_tmp> rels;
      
      for (map<wstring,int>::const_iterator rt = reltemp_id.begin(); rt != reltemp_id.end(); ++rt)
      {
        if(relations[rt->second].w1 == wid) rels.push_back(relations[rt->second]);
      }
      return rels;

    }
    
    std::vector<SG_relation_tmp> & semantic_graph::get_relations(const std::wstring &wid) {
      std::vector<SG_relation_tmp> rels;
      
      for (map<wstring,int>::iterator rt = reltemp_id.begin(); rt != reltemp_id.end(); ++rt)
      {
        if(relations[rt->second].w1 == wid) rels.push_back(relations[rt->second]);
      }
      return rels;
