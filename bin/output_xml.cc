...
void output_xml::PrintSemgraph(std::wostream &sout, const freeling::document &doc) const {
  ...
  //output temporal realtions
  for (vector<semgraph::SG_relation_tmp>::const_iterator rt=doc.get_semantic_graph().get_relations().begin();
       rt!=doc.get_semantic_graph().get_relations().end();
       rt++) {
      
      sout << L" <relation_tmp id=\"" << rt->get_id() << L"\" " 
         << L"w1=\"" << rt->get_w1() << L"\" "
         << L"w2=\"" << rt->get_w2() << L"\" " 
         << L"relation=\"" << rt->get_rel() << L"\" "
         << ">" << endl;
      
      sout << L" </relation_tmp>" << endl;
  }
